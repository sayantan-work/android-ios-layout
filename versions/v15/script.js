...
SCREENS['announcements'] = {
  ...
  render: () => {
    ...
    const tagStyle = (t) => {
      if (t === 'TOWNHALL')    return 'background:var(--ink);color:var(--sage)';
      if (t === 'HR')          return 'background:var(--ibm-soft);color:var(--ibm-deep)';
      if (t === 'BENEFITS')    return 'background:var(--crt-soft);color:var(--crt-deep)';
      if (t === 'POLICY')      return 'background:var(--amber-soft);color:#92400E';
      if (t === 'ANNOUNCEMENT')return 'background:var(--card);color:var(--text)';
      return 'background:var(--card);color:var(--text-2)';
    };
    const row = (a, isPinned) => `
      <li class="ann-row ${isPinned?'pinned':''}" onclick="toast('Announcement detail — v15 stub')">
        <div class="ann-h">
