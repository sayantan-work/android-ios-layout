...
function todayScreen() {
  const p = state.persona;
  const todayItems = ALL_TASKS.filter(t => t.section==='today');
  return `<div class="body-pad">
    <h1 class="h-screen">Today</h1>

    ${can(p,'punch') ? punchHero() : lockedCard('Punch &amp; attendance', 'Activate HRMS to track time at work.', 'About HRMS')}

    <h2 class="h-section first">${formatDate()} <span class="count">${todayItems.length}</span></h2>
    <div class="task-list">
      ${todayItems.map(taskRow).join('')}
    </div>

    ${can(p,'leaves.apply') || can(p,'salary.own') ? `
      <h2 class="h-section">Quick actions</h2>
      <div class="qa-list">
        ${can(p,'leaves.apply') ? `<div class="qa-row" data-nav="apply-request"><div class="qa-i">${I('i-leave')}</div><div class="qa-t">Apply request</div><span class="qa-c">Leave · OT · Advance</span></div>` : ''}
        ${can(p,'salary.own') ? `<div class="qa-row" data-nav="me-payroll"><div class="qa-i">${I('i-rupee')}</div><div class="qa-t">View payslip</div><span class="qa-c">May 2026</span></div>` : ''}
        ${can(p,'leaves.view-own') ? `<div class="qa-row" data-nav="me-leaves"><div class="qa-i">${I('i-leave')}</div><div class="qa-t">My leaves</div><span class="qa-c">12 left</span></div>` : ''}
        ${can(p,'attendance.own') ? `<div class="qa-row" data-nav="me-attendance"><div class="qa-i">${I('i-trend')}</div><div class="qa-t">Attendance</div><span class="qa-c">96%</span></div>` : ''}
        <div class="qa-row" data-nav="holidays"><div class="qa-i">${I('i-gift')}</div><div class="qa-t">Holidays</div><span class="qa-c">6 upcoming</span></div>
      </div>
    ` : ''}

    ${can(p,'announcements.view') ? `
      <div class="row-between"><h2 class="h-section" style="margin:0">From the company</h2><span class="see-all" data-nav="chat-channel" data-ctx-id="channel-announce">All ${I('i-chev-r')}</span></div>
      <div class="ann-track">
        <div class="ann-card" data-nav="announce-detail"><div class="ann-tag">All Branches · 7 May</div><div class="ann-title">Test 3</div><div class="ann-body-text">Test announcement content. Tap to read more.</div></div>
        <div class="ann-card" data-nav="announce-detail"><div class="ann-tag">HR · 5 May</div><div class="ann-title">Q2 OKRs published</div><div class="ann-body-text">Quarter goals are live. Sync with your manager.</div></div>
        <div class="ann-card" data-nav="announce-detail"><div class="ann-tag">Culture · 1 May</div><div class="ann-title">Foundation Day</div><div class="ann-body-text">All-hands lunch at the rooftop, Friday 1 PM.</div></div>
      </div>
    ` : ''}

    ${can(p,'announcements.view') ? `
      <h2 class="h-section">Birthdays today <span class="count">2</span></h2>
      <div class="bday-list">
        <div class="bday-row" data-nav="birthday" data-ctx-name="Rounik Tarafder"><div class="bd-av">RT</div><div class="bd-name">Rounik Tarafder</div><div class="bd-action">Send wish</div></div>
        <div class="bday-row" data-nav="birthday" data-ctx-name="Priya Sharma"><div class="bd-av">PS</div><div class="bd-name">Priya Sharma</div><div class="bd-action">Send wish</div></div>
      </div>
    ` : ''}
  </div>`;
}
function formatDate() {
  const d = new Date(2026, 4, 7); // May 7, 2026 (fixed for demo)
  const days = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
  const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  return `${d.getDate()} ${months[d.getMonth()]} · <span style="color:var(--red)">Today</span> · ${days[d.getDay()]}`;
}
...
function punchHero() {
  ...
  return `<div class="punch-block">
    <div class="punch-status ${state.punchedIn?'':'out'}"><span class="dot"></span>${status}${state.punchedIn?` · in at ${inTime}`:''}</div>
    <div class="punch-elapsed"><span class="pe-h">${elapsed}</span><span class="pe-of">/ 9h target</span></div>
    <div class="punch-bar"><div class="pb-fill" style="width:${fill}%"></div></div>
    <div class="punch-meta">
      <span><b>Flexible</b> hours</span>
      <span><b>Geofence</b> on</span>
      <span><b>OT</b> off</span>
    </div>
    <div class="punch-actions">
      <button class="punch-btn ${state.punchedIn?'out':'in'}" data-act="punch-hold">${I('i-power')} ${inLabel} · hold<span class="hold-fill"></span></button>
      <button class="punch-aux" data-nav="punch-location">${I('i-pin')} Map</button>
    </div>
  </div>`;
}
...
function taskRow(t) {
  ...
  const priority = t.kind === 'task' ? 'p2' : t.kind === 'meet' ? 'p3' : t.kind === 'reminder' ? '' : '';
  const trail = t.kind === 'meet'
    ? `<div class="t-join" data-nav="meet-detail" data-ctx-title="${t.title}">Join</div>`
    : '';
  return `<div class="task-row ${done?'done':''}" data-nav="${t.kind==='meet'?'meet-detail':'task-detail'}" data-ctx-title="${t.title}">
    <div class="check ${priority}" data-act="toggle-task" data-task="${t.id}" data-stop="1"></div>
    <div class="t-body">
      <div class="t-title">${t.title}</div>
      <div class="t-meta">
        ${t.when ? `<span class="t-due ${t.when.includes('Tomorrow')?'tomorrow':''}">${t.when}</span>` : ''}
        ${t.kind === 'task' ? '<span class="t-project"><span class="proj-dot"></span>#WORK</span>' : ''}
        ${t.kind === 'meet' ? '<span class="t-project"><span class="proj-dot" style="background:var(--p3)"></span>Calendar</span>' : ''}
        ${t.kind === 'reminder' ? '<span class="t-project"><span class="proj-dot" style="background:var(--text-3)"></span>Reminder</span>' : ''}
      </div>
    </div>
    ${trail}
  </div>`;
...
function tasksScreen() {
  ...
  return `<div class="body-pad">
    ...
    <div class="add-bar" data-nav="task-new">
      <div class="add-i">${I('i-plus')}</div>
      <div class="add-l">Add task</div>
    </div>
    ${['today','tomorrow','this-week'].map(sec => groups[sec].length ? `
      <h2 class="h-section ${sec==='today'?'first':''}">${sec==='today'?'Today':sec==='tomorrow'?'Tomorrow':'This week'} <span class="count">${groups[sec].length}</span></h2>
      <div>${groups[sec].map(taskRow).join('')}</div>
    ` : '').join('')}
    ${rows.length===0 ? `<div class="empty"><div class="empty-t">Looking sharp!</div><div class="empty-s">No items match this filter. Try another or add a new one.</div></div>` : ''}
...
function chatsScreen() {
  ...
  return `<div class="body-pad">
    ...
    ${rows.length===0 ? `<div class="empty"><div class="empty-t">Quiet day</div><div class="empty-s">No conversations match. Try a different filter or start a new chat.</div></div>` : ''}
    ${rows.map(c => `
...
function teamScreen() {
  ...
  if (!can(p,'team.view')) {
    return `<div class="body-pad">
      ...
      <h2 class="h-section first">Available tools <span class="count">locked</span></h2>
      <div class="manage-list">
        ${manageRow('i-check','Approvals','Approve & reject team requests',true)}
        ${manageRow('i-people','Register','See all employees in scope',true)}
        ${manageRow('i-trend','Reports','Attendance, payroll, leave',true)}
        ${manageRow('i-shield','BG Verification','Pre-hire checks',true)}
        ${manageRow('i-megaphone','Announce','Post company-wide updates',true)}
        ${manageRow('i-leave','Leaves','Branch leave management',true)}
        ${manageRow('i-bldg','Branches','Switch & manage branches',true)}
        ${manageRow('i-monitor','Web Admin','Bulk imports & audit logs',true)}
      </div>
    </div>`;
  }
  const isC = isCEO(p);
  const approvalCount = isC ? 5 : 4;
  return `<div class="body-pad">
    <h1 class="h-screen">Team</h1>
    ...
    <h2 class="h-section first">Pending approvals <span class="count">${approvalCount}</span></h2>
    ...
    <div style="padding:10px 0 4px"><a class="see-all" data-nav="approvals">View all ${approvalCount} approvals ${I('i-chev-r')}</a></div>

    <h2 class="h-section">Branch pulse</h2>
    ${branchPulse(isC)}

    <h2 class="h-section">Management tools</h2>
    <div class="manage-list">
      ${manageRow('i-people','Register', isC?'23 employees across branches':'18 employees · Head Office', false, 'register')}
      ${manageRow('i-trend','Reports', 'Attendance · Payroll · Leave', !can(p,'reports.branch') && !can(p,'reports.all'), 'reports')}
      ${manageRow('i-chart','Mgmt Review', '15 items need attention', false, 'mgmt-review', 15)}
      ${manageRow('i-shield','BG Verification', '2 in progress · 1 failed', false, 'bg-verify')}
      ${manageRow('i-megaphone','Announce', 'Post company-wide update', !can(p,'announcements.create'), 'announce-new')}
      ${manageRow('i-leave','Leaves', `${approvalCount} pending approvals`, false, 'leaves-queue')}
      ${manageRow('i-bldg','Branches', isC?'3 branches':'No other branches', !isC, isC?'branches-list':null)}
      ${manageRow('i-monitor','Web Admin', 'Bulk imports, audit logs', !can(p,'web-admin'), 'web-admin')}
    </div>
  </div>`;
}

function manageRow(icon, title, meta, locked, navTarget=null, badge=null) {
  if (locked) {
    return `<div class="manage-row" data-act="locked-learn" data-toast="Locked — requires permissions">
      <div class="mr-i locked">${I(icon)}</div>
      <div class="mr-body"><div class="mr-title locked">${title}</div><div class="mr-meta">${meta}</div></div>
      <span class="pill">Locked</span>
    </div>`;
  }
  const navAttr = navTarget ? `data-nav="${navTarget}"` : 'data-act="noop"';
  return `<div class="manage-row" ${navAttr}>
    <div class="mr-i">${I(icon)}</div>
    <div class="mr-body"><div class="mr-title">${title}</div><div class="mr-meta">${meta}</div></div>
    ${badge?`<span class="mr-badge">${badge}</span>`:''}
    <div class="mr-chev">${I('i-chev-r')}</div>
  </div>`;
}

function approvalCard(r) {
  return `<div class="appr-row approval">
    <div class="appr-head">
  ...
  return `<div class="pulse-block">
    <div class="row-between" style="align-items:flex-end">
      ...
      <div style="text-align:right">
        <div style="font-size:22px;font-weight:700;color:var(--green-done);font-feature-settings:'tnum';letter-spacing:-0.02em">${Math.round(present/total*100)}%</div>
        <div style="font-size:11px;color:var(--text-3);font-weight:600">attendance</div>
      </div>
    </div>
    <div class="pulse-bar">
    ...
    <div class="pulse-legend">
      <span><span class="lg-d" style="background:var(--green-done)"></span>Present ${present}</span>
      <span><span class="lg-d" style="background:var(--p1)"></span>Absent ${absent}</span>
      <span><span class="lg-d" style="background:var(--p3)"></span>Leave ${leave}</span>
    </div>
  </div>`;
}
...
function meScreen() {
  ...
  const P = PERSONAS[p];
  return `<div class="body-pad">
    <h1 class="h-screen">Me</h1>

    <div class="profile-hero">
      <div class="av ph-av ${P.tier==='L1'?'tier-l1':''}">${P.initials}</div>
      <div style="flex:1;min-width:0">
        <div class="ph-name">${P.name}</div>
        <div class="ph-role">${P.role}</div>
        <div class="ph-pills">
          <span class="pill green">Verified</span>
          ${P.tier?`<span class="pill red">${P.tier}</span>`:''}
        </div>
      </div>
    </div>

    ${can(p,'salary.own')
      ? `<div class="payroll-block" data-nav="payslip-detail" data-ctx-month="May 2026">
          <div class="payroll-month">May 2026 ${I('i-chev-d')}</div>
          <div class="payroll-amt"><small>₹</small>${p==='E'?'4,82,000':p==='D'?'92,500':'7,302'}</div>
          <div class="payroll-label">Net pay · Credited 1st</div>
          <div class="payroll-split">
            <div>Earnings<b>${p==='E'?'₹5,12,000':p==='D'?'₹95,500':'₹7,452'}</b></div>
            <div>Deductions<b>${p==='E'?'₹30,000':p==='D'?'₹3,000':'₹150'}</b></div>
            <div>Next pay<b>24 days</b></div>
          </div>
          <span class="payroll-cta">View payslip ${I('i-chev-r')}</span>
        </div>`
      : lockedCard('Payroll', 'Activate HRMS to see your salary, payslips, and salary advances.')}

    <h2 class="h-section first">Records</h2>
    <div class="manage-list">
      ${can(p,'attendance.own') ? manageRow('i-trend','My Attendance','May · 96% · 6 of 7 days', false, 'me-attendance') : manageRow('i-trend','My Attendance','HRMS required', true)}
      ${can(p,'leaves.view-own') ? manageRow('i-leave','My Leaves','3 used · 12 remaining', false, 'me-leaves') : manageRow('i-leave','My Leaves','HRMS required', true)}
      ${can(p,'salary.own') ? manageRow('i-rupee','Payslips','5 months', false, 'me-payslips') : ''}
      ${can(p,'leaves.apply') ? manageRow('i-card','Apply Request','Leave · OT · Advance · Reimburse', false, 'apply-request') : ''}
    </div>

    <h2 class="h-section">Documents</h2>
    <div class="manage-list">
      ${manageRow('i-doc','My Documents','Payslips, ID proofs, agreements', false, 'documents')}
      ${can(p,'handbook') ? manageRow('i-book','Employee Handbook','Policies & guidelines', false, 'handbook') : manageRow('i-book','Employee Handbook','HRMS required', true)}
    </div>

    ${can(p,'company.settings') ? `
      <h2 class="h-section">Admin tools</h2>
      <div class="manage-list">
        ${manageRow('i-bldg','Company Settings','Policies, billing, integrations', false, 'company-settings')}
        ${manageRow('i-gear','Branch Settings','Hours, holidays, locations', false, 'branch-settings')}
      </div>
    ` : ''}

    <h2 class="h-section">Account</h2>
    <div class="manage-list">
      ${manageRow('i-gear','Settings','Theme, notifications, privacy', false, 'settings')}
      ${manageRow('i-info','About','Version 8.0 · Help', false, 'about')}
    </div>

    <button class="btn-block" style="margin-top:18px;color:var(--red);border-color:var(--line)" data-act="sign-out">${I('i-out')} Sign Out</button>
  </div>`;
}
...
SCREENS['chat-channel'] = {
  ...
  render: (p, ctx) => `<div class="body-pad">
    <p style="font-size:13px;color:var(--text-2);margin:0 0 14px">📢 Broadcast channel · Liberty Infospace</p>
    <div class="manage-list">
      <div class="manage-row" data-nav="announce-detail">
        <div class="mr-body"><div class="mr-title">Test 3</div><div class="mr-meta">All Branches · 7 May · 194 reads</div></div>
        <div class="mr-chev">${I('i-chev-r')}</div>
      </div>
      <div class="manage-row" data-nav="announce-detail">
        <div class="mr-body"><div class="mr-title">Q2 OKRs published</div><div class="mr-meta">HR · 5 May · 192 reads</div></div>
        <div class="mr-chev">${I('i-chev-r')}</div>
      </div>
      <div class="manage-row" data-nav="announce-detail">
        <div class="mr-body"><div class="mr-title">Foundation Day celebration</div><div class="mr-meta">Culture · 1 May · 188 reads</div></div>
        <div class="mr-chev">${I('i-chev-r')}</div>
      </div>
    </div>
  </div>`
...
function renderInfo() {
  ...
  info.innerHTML = `
    ...
    <section>
      <h3>v8 design DNA</h3>
      <p><b>Todoist's design system.</b> Single red accent (#DC4C3E), pure white surfaces, hairline-separated lists (no cards), system fonts, generous whitespace. Circle checkboxes ARE the interaction. Kept v7's IA: same 5 tabs for every persona, permissions gate content via dashed locked rows.</p>
    </section>
    <section>
      <h3>What to try</h3>
