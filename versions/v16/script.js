...
SCREENS['announcements'] = {
  ...
  render: () => {
    ...
    const tagStyle = (t) => {
      if (t === 'TOWNHALL')    return 'background:var(--gradient-brand);color:var(--ink);border-color:transparent';
      if (t === 'HR')          return 'background:rgba(34,211,238,0.10);color:var(--cyan);border-color:rgba(34,211,238,0.30)';
      if (t === 'BENEFITS')    return 'background:rgba(34,197,94,0.10);color:var(--forest);border-color:rgba(34,197,94,0.30)';
      if (t === 'POLICY')      return 'background:var(--amber-soft);color:var(--amber);border-color:rgba(245,158,11,0.30)';
      if (t === 'ANNOUNCEMENT')return 'background:rgba(251,146,60,0.10);color:var(--coral);border-color:rgba(251,146,60,0.30)';
      return 'background:var(--surface-2);color:var(--text-2);border-color:var(--line)';
    };
    const row = (a, isPinned) => `
      <li class="ann-row ${isPinned?'pinned':''}" onclick="toast('Announcement detail — v16 stub')">
        <div class="ann-h">
