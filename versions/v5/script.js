...
const trayItems = {
  ...
  B: [
    {icon:'i-card',      label:'Apply Request',tint:'yellow', nav:'apply-request'},
    {icon:'i-doc',       label:'Payslip',    tint:'green',  nav:'me-payslips'},
    {icon:'i-trend',     label:'Attendance', tint:'bronze', nav:'me-attendance'},
    {icon:'i-cal-sub',   label:'My Leaves',  tint:'peach',  nav:'me-leaves'},
    {icon:'i-gift',      label:'Holidays',   tint:'rose',   nav:'holidays'},
    {icon:'i-book',      label:'Handbook',   tint:'purple', nav:'handbook'},
    {icon:'i-pin',       label:'Location',   tint:'info',   nav:'punch-location'},
    {icon:'i-bell-add',  label:'Reminders',  tint:'teal',   action:'noop', toast:'Reminders'},
  ],
  D: [
    ...
    {icon:'i-card',      label:'Apply Request',tint:'teal',  nav:'apply-request'},
  ...
  E: [
    {icon:'i-check-seal',label:'Approvals',   tint:'yellow', badge:'5',  nav:'approvals'},
    {icon:'i-star',      label:'Mgmt Review', tint:'rose',   badge:'15', nav:'mgmt-review'},
    {icon:'i-people',    label:'Register',    tint:'info',   nav:'register'},
    {icon:'i-bldg',      label:'Branches',    tint:'teal',   nav:'branches-list'},
    {icon:'i-megaphone', label:'Announce',    tint:'peach',  nav:'announce'},
    {icon:'i-chart',     label:'Reports',     tint:'bronze', nav:'reports'},
    {icon:'i-card',      label:'Apply Request',tint:'green',  nav:'apply-request'},
    {icon:'i-pin',       label:'Location',    tint:'purple', nav:'punch-location'},
  ],
};
...
/* ─── CHAT — universal ─── */
const CHAT_ROWS = [
  {id:'ai', av:'gradient-ai', initials:'AI', title:'EasyDo AI', meta:'Got it — I\'ll handle it.', time:'5:06 PM', kind:'ai'},
  {id:'amulya', av:'av2', initials:'AS', title:'Amulya Sir', meta:'Sounds good, thanks.', time:'5:06 PM', kind:'dm'},
  {id:'liberty-kolkata', av:'av4', initials:'LK', title:'Liberty Kolkata Dev', meta:'Sukanta: deploying tonight 🚀', time:'5:06 PM', unread:2, kind:'group'},
  {id:'ui-testing', av:'av3', initials:'UT', title:'UI Testing', meta:'Priya: can someone review screen 3?', time:'4:34 PM', unread:5, kind:'group'},
  {id:'ios-testing', av:'av1', initials:'IT', title:'iOS Testing Group', meta:'Rounik joined the group', time:'Yesterday', unread:3, kind:'group'},
  {id:'bug-resolve', av:'av5', initials:'BR', title:'Bug Resolve 2025', meta:'Sankha Subhra left the group', time:'Yesterday', kind:'group'},
];
  ...
  return `
    ...
    ${rows.map(r => `
      <div class="list-row" data-nav="chat-thread" data-ctx-id="${r.id}">
        ...
        <div class="list-body"><div class="list-title">${r.title}</div><div class="list-meta">${r.meta}</div></div>
  ...
  return `
    ...
    ${!isProd ? `
      <div class="glass pay-card" data-nav="payslip-detail" data-ctx-month="May 2026" data-ctx-amount="${payAmount}">
        ...
        <div style="margin-top:10px;font-size:11px;color:var(--muted-2)">Next payslip in 24 days</div>
      </div>
...
function handleAction(a, el, e) {
  if (a === 'back') back();
    ...
    const route = approvalRoute(state.persona);
    const kind = el.dataset.kind || 'Request';
    toast(`${kind} · ${route.toast}`, {kind:'success'});
    back();
  }
...
const formScreen = (fields, submitLabel, toastMsg, opts = {}) => `
  <div style="padding:2px 0 80px">
    ...
    <div style="position:sticky;bottom:0;padding:14px 0 0;background:linear-gradient(to top,#0a0a0d 60%,transparent)">
      <button class="btn-block primary" data-action="${opts.requestKind ? 'submit-request' : 'submit-form'}" ${opts.requestKind ? `data-kind="${opts.requestKind}"` : `data-toast="${toastMsg}"`}>${submitLabel}</button>
    </div>
  </div>`;
...
SCREENS['me-requests'] = {
  ...
  render: () => {
    ...
    return `
      ...
      <div style="position:fixed;bottom:90px;right:24px"><div class="fab" data-nav="apply-request">${I('i-plus')}</div></div>
    `;
  }
};
...
/* Persona-aware approval routing helper */
function approvalRoute(p) {
  // L1 (E) — self-approves, top of chain
  if (p === 'E') return {
    label: 'Auto-approved',
    note: "You're at the top of the chain — your requests auto-approve as soon as you submit.",
    accent: 'success',
    toast: 'Submitted & auto-approved',
  };
  // L2 (D) — Branch Manager → L1
  if (p === 'D') return {
    label: 'Routes to L1',
    note: 'Routes to L1 — Amulya Sir (CEO). Typical turnaround under 24 hours.',
    accent: 'info',
    toast: 'Sent to L1 for review',
  };
  // L3 (B) — Employee → reporting manager
  return {
    label: 'Routes to manager',
    note: 'Routes to your reporting manager — Amulya Sir. You\'ll get a notification once it\'s reviewed.',
    accent: 'muted',
...
SCREENS['apply-request'] = {
  title: 'Apply Request',
  render: (p) => {
    const route = approvalRoute(p);
    const types = [
      {icon:'i-cal-sub', title:'Leave',         meta:'Casual · Sick · Earned', nav:'apply-leave'},
      {icon:'i-clock',   title:'Overtime',      meta:'Pre-approval for extra hours', nav:'apply-ot'},
      {icon:'i-rupee',   title:'Salary Advance',meta:'Recover from next payslip', nav:'advance'},
      {icon:'i-card',    title:'Reimbursement', meta:'Travel, meals, supplies', nav:'reimburse'},
      {icon:'i-pin',     title:'Work-from-home',meta:'Single day or extended', action:'noop', toast:'WFH request form'},
      {icon:'i-doc',     title:'Document Request',meta:'Letters, certificates', action:'noop', toast:'Document request form'},
    ];
    const accentColor = route.accent === 'success' ? '#34d399' : route.accent === 'info' ? '#93c5fd' : 'var(--muted)';
    const accentBg = route.accent === 'success' ? 'rgba(16,185,129,0.08)' : route.accent === 'info' ? 'rgba(59,130,246,0.08)' : 'rgba(255,255,255,0.03)';
    const accentBorder = route.accent === 'success' ? 'rgba(16,185,129,0.25)' : route.accent === 'info' ? 'rgba(59,130,246,0.25)' : 'var(--border)';
    return `
      <p class="subhead" style="margin:0 0 10px">Pick a request type</p>
      ${settingsList(types)}
      <div style="margin:14px 0 0;padding:10px 12px;border-radius:var(--r-md);background:${accentBg};border:1px solid ${accentBorder};display:flex;gap:10px;align-items:flex-start">
        <div style="color:${accentColor};margin-top:1px">${I('i-info')}</div>
        <div style="flex:1">
          <div style="font-size:11.5px;font-weight:600;color:${accentColor}">${route.label}</div>
          <div style="font-size:11px;color:var(--muted);line-height:1.5;margin-top:2px">${route.note}</div>
        </div>
      </div>
    `;
  }
...
SCREENS['apply-leave'] = {
  ...
  `, 'Submit Request', '', {requestKind:'Leave'})
};
...
SCREENS['apply-ot'] = {
  ...
  `, 'Request Overtime', '', {requestKind:'Overtime'})
};
...
SCREENS['advance'] = {
  ...
  `, 'Submit Request', '', {requestKind:'Advance'})
};
...
SCREENS['reimburse'] = {
  ...
  `, 'Submit Claim', '', {requestKind:'Reimbursement'})
};
...
const SAMPLE_MSGS = {
  ...
  'liberty-kolkata':[
    {dir:'in', text:'Standup at 2 PM today, share blockers ahead of time please 🙏', ts:'9:10 AM'},
    {dir:'in', text:'Anyone free to pair on the staging issue?', ts:'11:25 AM'},
    {dir:'out', text:'I\'m on it, give me 30.', ts:'11:30 AM'},
    {dir:'in', text:'Sukanta: deploying tonight 🚀', ts:'5:06 PM'},
  ],
  'ai':[
    {dir:'in', text:'Morning Sayantan! Your day looks busy — 3 items on the calendar and one thing waiting on you. Want me to summarise?', ts:'9:35 AM'},
    {dir:'out', text:'Yeah go ahead.', ts:'9:36 AM'},
    {dir:'in', text:'9 AM Design Sync with Sukanta · 11:30 review block · 2 PM Liberty Kolkata standup. Reply "remind" and I\'ll ping you 10 min before each.', ts:'9:36 AM'},
...
SCREENS['approvals'] = {
  ...
  render: (p) => {
    const isCEO = p === 'E';
    const rows = [
      // L2-manager row only visible to L1 (CEO)
    ...
    const leaves = rows.filter(r=>/Leave/.test(r.type)).length;
    const ot = rows.filter(r=>/Overtime/.test(r.type)).length;
    const adv = rows.filter(r=>/Advance/.test(r.type)).length;
    return `
      <div style="display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap">
        <span class="chip active">All (${total})</span>
        <span class="chip">Leaves (${leaves})</span>
        <span class="chip">OT (${ot})</span>
        <span class="chip">Advance (${adv})</span>
        ${isCEO ? '<span class="chip">From managers (1)</span>' : ''}
      </div>
      ${rows.map(r => `
        <div class="glass approval">
          <div class="approval-row">
            <div class="list-avatar ${r.av}" style="width:28px;height:28px;font-size:9px">${r.n.split(' ').map(x=>x[0]).join('').slice(0,2)}</div>
            <div style="flex:1"><div class="approval-name">${r.n}${r.tier?` <span class="pill" style="background:rgba(59,130,246,0.12);color:#93c5fd;margin-left:4px;font-size:8px;padding:2px 6px">${r.tier}</span>`:''}</div><div class="approval-meta">${r.meta}</div></div>
          </div>
